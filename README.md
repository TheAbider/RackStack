...
**Actions:** `Cleanup` (disk cleanup), `Debloat` (remove bloatware + telemetry), `HealthCheck` (system health report), `QuickScan` (health + disk + debloat analysis), `Inventory` (server inventory for CMDB), `DriftCheck` (configuration drift detection), `Snapshot` (performance metric capture), `Compliance` (unified health + readiness + drift report), `Harden` (CIS-lite security hardening audit), `Remediate` (auto-fix drifted settings), `Aggregate` (fleet-wide report aggregator), `Compare` (side-by-side host comparison), `Export` (comprehensive 8-section server profile), `Trend` (performance trend analysis over time), `CertCheck` (certificate expiry audit across 5 stores), `ReportHTML` (generate HTML reports: Health/Readiness/Trend), `ListeningPorts` (TCP listening port inventory), `SoftwareList` (installed software audit), `Uptime` (uptime + reboot history), `ServiceAudit` (key service status audit), `EventAudit` (critical/error event log summary), `NetInfo` (network adapter configuration), `Batch` (JSON-driven full config).
