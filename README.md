...
<p align="center">
  ...
  <img alt="4837 tests" src="https://img.shields.io/badge/tests-4837-brightgreen">
...
> **`RackStack.ps1`** is the **modular loader** (~130 lines). It dot-sources all 71 modules from `Modules/` and starts the tool. Use this for development -- edit individual module files, then run.
...
The output is **`RackStack v{version}.ps1`** -- a self-contained single file with all 71 modules baked in (version from `00-Initialization.ps1`). This is the file used to compile the `.exe`.
...
```
RackStack/
├── RackStack.ps1               # Modular loader -- dot-sources 71 modules (dev use)
...
├── Tests/
│   ├── Run-Tests.ps1           # 4,837 automated tests
...
71 modules numbered for load order. Dependencies flow downward.
...
```powershell
# Full test suite (4,837 tests, ~4 minutes)
powershell -ExecutionPolicy Bypass -File Tests\Run-Tests.ps1

# PSScriptAnalyzer (0 errors on all 71 modules + monolithic)
...
The sync script matches `#region`/`#endregion` markers between modules and the monolithic file. All 70 region pairs are flat (non-nested). Use `-DryRun` to preview.
