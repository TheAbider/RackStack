...
**Actions:** `Cleanup` (disk cleanup), `Debloat` (remove bloatware + telemetry), `HealthCheck` (system health report), `QuickScan` (health + disk + debloat analysis), `Inventory` (server inventory for CMDB), `DriftCheck` (configuration drift detection), `Snapshot` (performance metric capture), `Compliance` (unified health + readiness + drift report), `Harden` (CIS-lite security hardening audit), `Remediate` (auto-fix drifted settings), `Aggregate` (fleet-wide report aggregator), `Compare` (side-by-side host comparison), `Export` (comprehensive 11-section server profile), `Trend` (performance trend analysis over time), `CertCheck` (certificate expiry audit across 5 stores), `ReportHTML` (generate HTML reports: Health/Readiness/Trend), `ListeningPorts` (TCP listening port inventory), `SoftwareList` (installed software audit), `Uptime` (uptime + reboot history), `ServiceAudit` (key service status audit), `EventAudit` (critical/error event log summary), `NetInfo` (network adapter configuration), `ScheduledExport` (register/manage automated Export scheduled task), `ValidateConfig` (pre-flight validation of batch config files), `Watch` (threshold-based monitoring health gate), `Query` (search fleet export archive), `Diff` (full-profile change detection between exports), `Baseline` (capture known-good Export profile), `Alert` (detect + notify via webhook/email/event log), `FleetScan` (run any action across fleet via WinRM), `PatchStatus` (patch currency + pending reboot detection), `UserAudit` (local account security hygiene audit), `FirewallAudit` (firewall profile + rule analysis), `TaskAudit` (scheduled task health check), `DiskAudit` (physical disk health + volume utilization), `TLSAudit` (TLS/SSL protocol configuration audit), `SMBAudit` (SMB protocol + share security audit), `DriverAudit` (driver signing + inventory audit), `TimeAudit` (NTP sync + time drift detection), `BootAudit` (firmware + secure boot + reboot status), `GPOAudit` (applied Group Policy inventory), `MemoryAudit` (RAM + DIMM + page file audit), `ProcessAudit` (top CPU/memory + unsigned process detection), `BackupAudit` (VSS writers + shadow copies + WSB status), `ShareAudit` (SMB + NTFS share permissions), `DNSAudit` (DNS config + resolution tests), `PowerAudit` (power plan + sleep configuration), `RegistryAudit` (security hardening baseline), `ProfileAudit` (user profile size + staleness), `HyperVAudit` (VM inventory + checkpoints + replication), `NetworkAudit` (adapter config + IP + routing), `StorageAudit` (storage pools + virtual disks), `FeatureAudit` (installed roles + features inventory), `AutoStartAudit` (registry Run keys + startup items + auto services), `BIOSAudit` (BIOS/firmware + serial numbers + baseboard), `ClusterAudit` (failover cluster nodes + resources), `AuditPolicyAudit` (Windows security audit policy configuration), `EnvAudit` (environment variables + PATH analysis), `CrashAudit` (BSOD/crash dump history), `LocalGroupAudit` (local groups + membership), `WMIAudit` (WMI repository health), `TempAudit` (temp files + reclaimable space), `UpdatePolicyAudit` (Windows Update policy + WSUS), `IISAudit` (IIS sites + app pools), `SSHAudit` (OpenSSH server config + keys), `BitLockerAudit` (encryption status + recovery keys), `PrintAudit` (printer inventory + queue status), `CredGuardAudit` (Credential Guard + VBS + HVCI), `PortAudit` (outbound connectivity tests), `AntivirusAudit` (Defender + AV status + signatures), `DotNetAudit` (.NET Framework + Runtime versions), `RDPAudit` (RDP config + NLA + sessions), `VPNAudit` (VPN connections + RRAS), `HostsFileAudit` (hosts file + hijack detection), `NetStatAudit` (established TCP connections), `LicenseAudit` (Windows activation status), `USBDeviceAudit` (USB devices + storage policy), `AppLockerAudit` (AppLocker policy + rules), `EventSubAudit` (WMI event subscriptions/persistence), `HotfixAudit` (installed KB inventory), `SysInfoAudit` (comprehensive system snapshot), `LogonAudit` (recent logons + failed attempts), `ACLAudit` (critical folder permissions), `RecoveryAudit` (restore points + recovery), `ServiceAccountAudit` (services with custom accounts), `ProxyAudit` (system/WinHTTP proxy config), `PendingRebootAudit` (comprehensive reboot detection), `PageFileAudit` (page file config + utilization), `CPUAudit` (CPU topology + utilization), `Batch` (JSON-driven full config).
