...
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingConvertToSecureStringWithPlainText', '', Justification='Operator-supplied secrets (Azure Arc SP secret, test fixtures) arrive as plaintext at the boundary; ConvertTo-SecureString is the securing step, paired with prompt scrub via Clear-SecureMemory')]
param(
    ...
    # Emit newline-delimited JSON events for long-running actions instead of a single
    # summary at the end. Works in combination with -OutputFormat JSON. Consumers can
    # parse line-by-line to drive progress UIs or pipeline orchestration.
    [switch]$Stream
)
