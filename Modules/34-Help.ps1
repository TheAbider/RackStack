...
function Show-Help {
    ...
    Write-OutputColor "  ├────────────────────────────────────────────────────────────────────────┤" -color "Info"
    Write-OutputColor "  │$("  [1] Network Config          Physical NIC / Hyper-V host / iSCSI SAN".PadRight(72))│" -color "Success"
    Write-OutputColor "  │$("      > Physical Adapter:  IP, DNS, Rename, Disable IPv6".PadRight(72))│" -color "Info"
    Write-OutputColor "  │$("      > Host (Hyper-V):    Virtual switches, SET, host vNICs".PadRight(72))│" -color "Info"
    Write-OutputColor "  │$("      > Storage & SAN:     iSCSI connectivity".PadRight(72))│" -color "Info"
...
function Search-HelpTopics {
    ...
    $topics = @(
        ...
        @{ Title = "Hyper-V & VM Audits"; Keywords = @("hypervaudit", "vm-audit", "vm-snapshot", "vm-inventory", "live-migration", "vmovercommit", "vmwaste", "replica-audit", "virtual-switch"); Description = "Hyper-V/VM audits: HyperVAudit, VMInventoryExport, VMSnapshotAudit, VMOvercommitAudit, VMResourceWaste, ReplicaLagAudit, LiveMigrationAudit, VirtualSwitchAudit." }
        @{ Title = "System Audits"; Keywords = @("sysinfo", "process-audit", "cpu-audit", "memory-audit", "pagefile", "event-audit", "eventlog", "scheduled-task", "scheduled-reboot", "crash", "bsod", "drivers", "hotfix", "patch", "update", "licensing", "feature-audit", "autostart", "usb", "bios", "driver-audit"); Description = "System audits: SysInfoAudit, CPUAudit, MemoryAudit, PageFileAudit, ProcessAudit, HandleLeakAudit, EventAudit, EventSubAudit, EventLogCapacityAudit, TaskAudit, TaskHistoryAudit, ScheduledRebootAudit, CrashAudit, DriverAudit, HotfixAudit, PatchStatus, LicenseAudit, FeatureAudit, AutoStartAudit, USBDeviceAudit, BIOSAudit, BackupAudit, RecoveryAudit, WMIAudit, BootAudit, PendingRebootAudit, PowerAudit, PowerShellAudit, TokenPrivilegeAudit, WindowsCapabilityAudit, UpdatePolicyAudit, WindowsUpdateAudit, TimeAudit, TimeSkewAudit, LocaleAudit, EnvAudit." }
        @{ Title = "Fleet & Reporting"; Keywords = @("fleet", "fleet-scan", "fleet-report", "aggregate", "compare", "diff", "baseline", "baseline-diff", "rotate", "exports", "report-html", "scheduled-export", "watch", "query", "alert"); Description = "Fleet and reporting: FleetScan (run any action across fleet via WinRM), FleetReport (aggregate from multiple servers), Aggregate, Compare, Diff, Baseline, BaselineDiff, ReportHTML, ScheduledExport, RotateExports, Watch (threshold gate), Query (search archive), Alert (webhook/email)." }
        @{ Title = "Server Roles Audit"; Keywords = @("rds", "iis", "ssh", "print", "sccm", "scom", "azuread", "intune", "wac", "admin-center", "domain-trust", "secure-channel", "gpo", "gpresult"); Description = "Role-specific audits: IISAudit, SSHAudit, PrintAudit, RDPAudit, SCCMClientAudit, SCOMAgentAudit, WACConnectivityAudit, AzureADAudit, DomainTrustAudit, SecureChannelAudit, GPOAudit, GPResultAudit, StartupScriptAudit, ComObjectAudit, ServiceAccountAudit, ServiceRecoveryAudit." }
        @{ Title = "Scores & Dashboards"; Keywords = @("score", "health-score", "readiness", "healthdashboard", "server-score", "storage-score", "cluster-score", "policycheck", "validate-change", "netmap", "sla"); Description = "Unified scores/dashboards: ServerScore, HealthDashboard, ClusterHealthScore, StorageHealthScore, Readiness, PolicyCheck (custom compliance), NetMap (dependency inventory), SLAReport (uptime compliance), Validate (pre/post change diff)." }
    )
