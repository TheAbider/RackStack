...
function Search-HelpTopics {
    ...
    $topics = @(
        ...
        @{ Title = "CLI Actions"; Keywords = @("cli", "action", "headless", "automation", "fleet", "json", "audit", "scan", "score", "dashboard", "monitor"); Description = "149 CLI actions for headless automation. Run -ListActions to see all. JSON output via -OutputFormat JSON. Key: ServerScore, HealthDashboard, FleetReport, ClusterHealthScore, StorageHealthScore." }
    )
