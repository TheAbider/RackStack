...
function Search-HelpTopics {
    ...
    $topics = @(
        ...
        @{ Title = "CLI Actions"; Keywords = @("cli", "action", "headless", "automation", "fleet", "json", "audit", "scan", "score", "dashboard", "monitor", "policy", "sla", "netmap", "validate"); Description = "167 CLI actions for headless automation. Run -ListActions to see all. JSON output via -OutputFormat JSON. Key: PolicyCheck, SLAReport, NetMap, Validate, ServerScore, HealthDashboard, FleetReport." }
    )
