...
try {
    ...
    # Export CLI action tests (v1.33.0)
    ...
    Write-TestResult "50-EntryPoint: Export calls Show-SystemHealthCheck" ($mod50 -match "'Export'[\s\S]{0,1000}Show-SystemHealthCheck")
    Write-TestResult "50-EntryPoint: Export calls Get-ServerInventory" ($mod50 -match "'Export'[\s\S]{0,2000}Get-ServerInventory")
    Write-TestResult "50-EntryPoint: Export calls Get-SecurityHardeningChecks" ($mod50 -match "'Export'[\s\S]{0,3000}Get-SecurityHardeningChecks")
    Write-TestResult "50-EntryPoint: Export calls Save-PerformanceSnapshot" ($mod50 -match "'Export'[\s\S]{0,4000}Save-PerformanceSnapshot")
    Write-TestResult "50-EntryPoint: Export calculates hardening score" ($mod50 -match "'Export'[\s\S]{0,3500}hardenScore")
    Write-TestResult "50-EntryPoint: Export JSON has Health key" ($mod50 -match "'Export'[\s\S]{0,6000}Health\s*=")
    Write-TestResult "50-EntryPoint: Export JSON has Inventory key" ($mod50 -match "'Export'[\s\S]{0,6000}Inventory\s*=")
    Write-TestResult "50-EntryPoint: Export JSON has Hardening key" ($mod50 -match "'Export'[\s\S]{0,6000}Hardening\s*=")
    Write-TestResult "50-EntryPoint: Export JSON has Snapshot key" ($mod50 -match "'Export'[\s\S]{0,6500}Snapshot\s*=")
    Write-TestResult "50-EntryPoint: Export JSON output" ($mod50 -match "'Export'[\s\S]{0,7000}ConvertTo-Json")
