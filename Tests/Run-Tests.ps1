...
try {
    ...
    # Export CLI action tests (v1.33.0, enhanced v1.36.0 — 8-section comprehensive profile)
    ...
    Write-TestResult "50-EntryPoint: Export step 1 health check" ($mod50 -match "'Export'[\s\S]{0,500}\[1/8\][\s\S]{0,200}Show-SystemHealthCheck")
    Write-TestResult "50-EntryPoint: Export step 2 inventory" ($mod50 -match "'Export'[\s\S]{0,1500}\[2/8\][\s\S]{0,200}Get-ServerInventory")
    Write-TestResult "50-EntryPoint: Export step 3 hardening" ($mod50 -match "'Export'[\s\S]{0,2500}\[3/8\][\s\S]{0,200}Get-SecurityHardeningChecks")
    Write-TestResult "50-EntryPoint: Export step 4 snapshot" ($mod50 -match "'Export'[\s\S]{0,3500}\[4/8\][\s\S]{0,200}Save-PerformanceSnapshot")
    Write-TestResult "50-EntryPoint: Export step 5 certificates" ($mod50 -match "'Export'[\s\S]{0,5000}\[5/8\][\s\S]{0,200}certificates")
    Write-TestResult "50-EntryPoint: Export step 6 listening ports" ($mod50 -match "'Export'[\s\S]{0,8000}\[6/8\][\s\S]{0,200}listening ports")
    Write-TestResult "50-EntryPoint: Export step 7 software" ($mod50 -match "'Export'[\s\S]{0,10000}\[7/8\][\s\S]{0,200}software")
    Write-TestResult "50-EntryPoint: Export step 8 uptime" ($mod50 -match "'Export'[\s\S]{0,12000}\[8/8\][\s\S]{0,200}uptime")
    Write-TestResult "50-EntryPoint: Export calculates hardening score" ($mod50 -match "'Export'[\s\S]{0,3500}hardenScore")
    Write-TestResult "50-EntryPoint: Export scans cert stores" ($mod50 -match "'Export'[\s\S]{0,6000}Cert:\\LocalMachine\\My")
    Write-TestResult "50-EntryPoint: Export queries TCP connections" ($mod50 -match "'Export'[\s\S]{0,10000}Get-NetTCPConnection.*Listen")
    Write-TestResult "50-EntryPoint: Export scans registry software" ($mod50 -match "'Export'[\s\S]{0,12000}Wow6432Node")
    Write-TestResult "50-EntryPoint: Export uses CIM uptime with timeout" ($mod50 -match "'Export'[\s\S]{0,14000}Invoke-WithTimeout")
    Write-TestResult "50-EntryPoint: Export checks event ID 6008" ($mod50 -match "'Export'[\s\S]{0,16000}6008")
    Write-TestResult "50-EntryPoint: Export JSON has Health key" ($mod50 -match "'Export'[\s\S]{0,20000}Health\s*=")
    Write-TestResult "50-EntryPoint: Export JSON has Inventory key" ($mod50 -match "'Export'[\s\S]{0,20000}Inventory\s*=")
    Write-TestResult "50-EntryPoint: Export JSON has Hardening key" ($mod50 -match "'Export'[\s\S]{0,20000}Hardening\s*=\s*@")
    Write-TestResult "50-EntryPoint: Export JSON has Snapshot key" ($mod50 -match "'Export'[\s\S]{0,20000}Snapshot\s*=")
    Write-TestResult "50-EntryPoint: Export JSON has Certificates key" ($mod50 -match "'Export'[\s\S]{0,20000}Certificates\s*=\s*@")
    Write-TestResult "50-EntryPoint: Export JSON has ListeningPorts key" ($mod50 -match "'Export'[\s\S]{0,20000}ListeningPorts\s*=\s*@")
    Write-TestResult "50-EntryPoint: Export JSON has Software key" ($mod50 -match "'Export'[\s\S]{0,20000}Software\s*=\s*@")
    Write-TestResult "50-EntryPoint: Export JSON has Uptime key" ($mod50 -match "'Export'[\s\S]{0,20000}Uptime\s*=\s*@")
    Write-TestResult "50-EntryPoint: Export JSON output" ($mod50 -match "'Export'[\s\S]{0,22000}ConvertTo-Json")
